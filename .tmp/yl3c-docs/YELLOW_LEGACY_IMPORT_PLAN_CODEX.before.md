...
**Status:** YL-3B is implemented in the current worktree atop the YL-3A source state and awaits runtime review and user commit. Stop before YL-3C.
...
| YL-3C | 11 card transition; 17 font cleanup, in separate patches | Luna stages small changes after main graphics review | DMG/CGB entry/exit and EXP/status tile tests; WIP checkpointed |
