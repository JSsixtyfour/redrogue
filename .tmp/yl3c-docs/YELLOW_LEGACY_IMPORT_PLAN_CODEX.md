...
**Status:** YL-3C item 11 is implemented in the current worktree atop user commit `18963f7a`; runtime review and user commit are pending. Stop before YL-3C item 17.
...
4. Design encounter-source aggregation for procedural stages, authored encounters, gifts, and fishing. Decide "possible in the game" versus "available in this run"; do not display false fixed Kanto locations. Bound/deduplicate the output and use a useful source-list UI if a town map is misleading.
...
| YL-3C | 11 card transition; 17 font cleanup, in separate patches | Item 11 implemented; item 17 remains stopped pending commit | Item 11 builds pass and focused tests pass 2/2; smoke 244/245 with one unrelated concurrent-work AI failure; runtime queue pending |
| YL-4A | 1 Ghost/Water/Rock + shiny markers | Luna audits state/tiles; main presents layout and integrates | Correct flags, overlap/placement reviewed; no tile collision |
