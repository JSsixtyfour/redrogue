...
## 2026-09-09 Yellow Legacy YL-4A - battle variant markers and evolution preservation

Measured from fresh forced Red, Blue, and Blue Debug builds at user commit `9dd05c3b`
plus the uncommitted YL-4A item 1 patch.

- `PlaceHUDTiles` adds one shared farcall after drawing either HUD. The bank `$2F`/Debug
  `$35` helper reads the active player or enemy struct, then writes one trailing marker:
  Water, Rock, Dragon, generic type variant, Ghost, or shiny. Type variant takes priority
  over Ghost, which takes priority over shiny. Font tiles `$D0-$D5` replace blank slots
  inside the existing 128-tile font and do not change `Font Graphics` size.
- `EvolutionAfterBattle`, the common evolution lifecycle, preserves an instance's stored
  `MON_TYPE2` across the ordinary `SetPartyMonTypes` reset when `BIT_TYPE_VARIANT` is set.
  Ordinary evolutions remain unchanged. Bank `$17` stays size-neutral because its pinned
  `Pics 11` section leaves only 2 bytes free.
- Bank `$05` `Battle Engine 2` grows exactly 8 bytes to `$07F0` at `$6340-$6B2F`;
  free space falls from `$000D` / 13 to `$0005` / 5 bytes in all targets.
- Bank `$31` `Evos Moves` grows `$0022` / 34 bytes from `$1E56` to `$1E78` at
  `$4000-$5E77`; free space falls from `$21AA` / 8,618 to `$2188` / 8,584 bytes.
- The floating `rogue` section grows `$004B` / 75 bytes to `$37EC` in Red/Blue and
  `$392A` in Debug. Fresh free space is `$00CF` / 207 bytes in Red/Blue bank `$2F`
  and `$06D6` / 1,750 bytes in Debug bank `$35`.
- No section moved banks. The new bank `$05` call uses the established farcall; `d`, `e`,
  and flags survive `Bankswitch`, so the side discriminator in `e` reaches the helper.
  No direct cross-bank data read, pointer table, `BANK()` assumption, section fallthrough,
  inline bank switch, WRAM, HRAM, SRAM, union, save-layout, or VRAM-layout declaration changed.
- ROM MD5: Red `AC813FA79BB2DD66CDE8E85AF2A30CF7`; Blue
  `B1C2FE080C54DF27B049866336AD4DC9`; Blue Debug
  `95DD5CD74A9AC49EE4235105C8647205`.
- Fresh forced builds pass and focused marker/evolution contracts pass 3/3. Ordinary smoke
  passes 250/251; only the already-known debug AI-injection smoke test fails. YL-4A does
  not touch AI/debug injection (~99% confident). Per user direction, `make ai_scenarios`
  was not run and that failure was not investigated.
- Runtime acceptance remains pending. Marker presentation, redraw lifecycle, state priority,
  type-preserving evolution routes, save/box persistence, and DMG/CGB tile inspection are
  queued at the absolute bottom of `YELLOW_LEGACY_IMPORT_PLAN_CODEX.md`.

## 2026-09-09 Yellow Legacy YL-3C item 17 - bold-P font consolidation

Measured from fresh forced Red, Blue, and Blue Debug builds in the current worktree at user
commit `697e53db` plus the uncommitted item 17 patch. The worktree also contains ongoing,
user-owned Regional Forms changes; those changes are preserved and are not attributed to this
checkpoint.

- `gfx/font/font_battle_extra.png` now contains the existing bold-P glyph in source tile 16,
  which the composite loader places at `vChars2` tile `$72`. The unused Japanese opening-quote
  charmap alias is replaced with `<BOLD_P>`, the status screen's separate `PTile` copy is removed,
  and redundant tracked `gfx/font/P.png` is deleted.
- The VRAM alias remains intentional: status PP renders `<BOLD_P> = $72`; battles overwrite
  `$72` with `<EXP_BAR_FULL>` and `$70` with `<EXP_BAR_PARTIAL>`. All five battle reload calls to
  `CalcAndLoadExpBarDynamicTile` remain. Optional battle PP display was not imported.
- `Font Graphics` remains `$1558` / 5,464 bytes at `$4F00-$6457`, size-neutral. `Battle Engine 1`
  shrinks from `$12A4` / 4,772 bytes to `$1290` / 4,752 bytes at `$6458-$76E7`, reclaiming exactly
  `$0014` / 20 bytes: a 12-byte copy sequence and the standalone 8-byte tile.
- Concurrent Regional Forms work adds 13 bytes elsewhere in bank `$04`, so combined-worktree
  bank free space rises from `$000C` / 12 bytes to `$0013` / 19 bytes rather than by the full
  20-byte item 17 reclamation. No section moved banks.
- No plain cross-bank call, direct cross-bank data read, pointer table, `BANK()` assumption,
  fallthrough, inline bank switch, RAM, HRAM, SRAM, union, save-layout, or VRAM-layout declaration
  changed.
- Artifact scope: these hashes identify combined-worktree artifacts containing ongoing Regional
  Forms work; they are not pure YL-3C item 17 artifacts. ROM MD5: Red
  `CC34AD3E01C2D685D8FEA4EE8A6A3E2D`; Blue `AABF50531C1BDCF422C58CD8B92146FB`;
  Blue Debug `67DE8505433349EE20EA62708E26A368`.
- Fresh forced Red/Blue/Debug builds pass and focused source/asset contracts pass 3/3. Ordinary
  smoke passes 247/248; the only failure is the already-known debug AI-injection smoke test in
  the concurrent-work artifact. Per user direction, `make ai_scenarios` was not run and the
  unrelated AI failure was not investigated.
- Runtime acceptance remains pending. Status PP, EXP-bar restoration, Pokedex `$72` reuse,
  repeated battle/status transitions, DMG/CGB, and live tilemap/VRAM inspection are queued at the
  bottom of `YELLOW_LEGACY_IMPORT_PLAN_CODEX.md`.

## 2026-09-09 Yellow Legacy YL-3C item 11 - Trainer Card DMG settling delays

Measured from fresh forced Red, Blue, and Blue Debug builds in the current worktree at user
commit `18963f7a` plus the uncommitted item 11 patch. The worktree also contains ongoing,
user-owned Regional Forms changes; those changes are preserved and are not attributed to this
checkpoint.

- `engine/menus/start_sub_menus.asm` adds the pinned donor's three-frame wait after the Trainer
  Card palette command and after map reload, before each reveal. Both waits are gated by
  `wOnSGB == 0`. Red Rogue raises `wOnSGB` on either SGB or CGB, so only the DMG path waits.
- Existing appearance-aware drawing, badges, menu-buffer restoration, CGB palette/attribute
  handling, `hTileAnimations` save/restore, map reload, and Start-menu redisplay remain in their
  original order. No broad palette/fade refactor or graphics-data change was made.
- Bank `$04` section `Battle Engine 1` grows from `$1296` / 4,758 bytes to `$12A4` / 4,772
  bytes at `$6458-$76FB`, adding exactly `$000E` / 14 bytes. Bank `$04` free space falls from
  `$001A` / 26 bytes to `$000C` / 12 bytes in Red, Blue, and Blue Debug.
- No section moved banks. The added calls target the existing HOME `Delay3` routine. No direct
  cross-bank data read, pointer table, `BANK()` assumption, fallthrough, inline bank switch,
  RAM, HRAM, SRAM, union, or save-layout declaration changed.
- Artifact scope: these hashes identify combined-worktree artifacts containing ongoing Regional
  Forms work; they are not pure YL-3C item 11 artifacts. ROM MD5: Red
  `844A297E520F04C3D72403962E2A8E6D`; Blue `24A69EE7E2EA5DCC8CFE666C71A69A5A`;
  Blue Debug `2DFC81FF5B8D065F05440522F7495C23`.
- Fresh forced Red/Blue/Debug builds pass and focused source-contract tests pass 2/2. Full smoke
  passes 244/245: both item 11 tests pass, while the unrelated debug AI injection test repeatedly
  returns no score records in the concurrent-work artifact. The Trainer Card patch does not touch
  battle or debug injection code, so that failure is unrelated to item 11 (~99% confident).
- Runtime acceptance remains pending. Frame-step DMG/CGB entry and exit, repeated use, every
  player appearance, and live tilemap/VRAM/CGB-attribute inspection are queued at the bottom of
  `YELLOW_LEGACY_IMPORT_PLAN_CODEX.md`.
...
## 2026-09-09 Yellow Legacy YL-4B Pokedex MOVE viewer

Measured from fresh forced Red, Blue, and Blue Debug builds at user commit `6aceacb9` plus the uncommitted YL-4B implementation and concurrent user-owned Pokemon/reward work.

- Bank `$10` section `Pokedex` grows exactly **`$0009` / 9 bytes**, from `$3067` to **`$3070`**, because the old three-byte AREA predef is replaced by an internal-species load and an eight-byte farcall. Fresh bank `$10` free space is **`$01A3` / 419 bytes** in all three variants.
- Bank `$2C` section `Status View Navigation` grows exactly **`$00BF` / 191 bytes**, from `$079C` to **`$085B`**. This includes the dedicated three-view viewer, its stale-fusion/form containment, the status-glyph reload, and a four-byte refactor that exposes the existing status-graphics loader. Fresh bank `$2C` free space is **`$03BB` / 955 bytes** in Red and **`$03CB` / 971 bytes** in Blue and Blue Debug.
- The bank `$10` caller passes the already-converted internal species ID in farcall-safe register `e`. The viewer remains in bank `$2C` with the existing Learndex renderer and data walkers. No plain cross-bank call/read, pointer-table width change, `BANK()` assumption, fallthrough across sections, or inline ROMX bank switch was introduced (~99% confident).
- The viewer saves/restores tile-animation state and temporarily clears/restores `wLoadedMon + MON_CATCH_RATE` plus `wFormContextSpecies`, `wFormContextForm`, `wMonHForm`, and `wMonHFormSpecies`, so TM/HM compatibility cannot inherit a stale loaded fusion secondary and base-species headers cannot inherit a stale regional-form refresh (~97% confident). The Pokedex's established `b = 0` return path rebuilds its graphics and list state.
- No section moved banks and YL-4B adds no WRAM, HRAM, SRAM, union, save-layout, or VRAM-layout declaration. Current user-owned `ram/wram.asm` changes are excluded.
- User runtime testing found Pokédex artwork tile `$6E` rendered where the level list expects `<LV>`. The viewer now calls the established status-glyph loader before drawing, and START cycles LEVEL UP, TM/HM, and TUTOR (~98% confident).
- Fresh forced Red, Blue, and Debug builds pass. MD5: Red `85BB9508B1D83397DA227C1380AE8E5E`, Blue `5B5D90EAD3902965D3A3C31A4F7A07B7`, Debug `751640E99C82790CD99F2AE198323551`. Focused YL-4B contracts pass **2/2**. Ordinary smoke reached and passed them before the review fix; it was then interrupted after stalling at the known debug AI-injection test. The final rendering/tutor build passes all three variants; no AI scenario suite or investigation was run by user direction.

Pending BGB acceptance: corrected `<LV>` glyphs, level-up/TM-HM/tutor content, three-view START cycle, cursor limits, internal-species correctness, stale-fusion/form isolation, and clean A/B return to the same Pokedex selection on DMG and CGB.
