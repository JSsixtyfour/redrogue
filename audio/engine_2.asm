; The second of four audio banks. Formerly a full duplicated engine
; (UpdateMusic/PlayNextNote/the whole note-command interpreter); that logic
; now lives once in engine_1.asm (AUDIO_1), reached via farcall/GetNextMusicByte
; (see home/audio.asm). This bank keeps only its own PlaySound entry point,
; because SFX/music headers (SFX_Headers_2) are still per-bank data. The old
; low-health-alarm / battle-SFX tempo-and-frequency-modifier logic that used to
; live here (Audio2_PlayNextNote/Audio2_SetSfxTempo/Audio2_ApplyFrequencyModifier/
; Audio2_IsBattleSFX) also moved into engine_1.asm's single copy, now bank-aware
; via [wAudioROMBank] instead of implicitly "this IS bank 2".
;
; Audio2_InitMusicVariables/Audio2_InitSFXVariables/Audio2_StopAllAudio below
; are exported (global, not local `.playMusic`-style labels) because AUDIO_1's
; engine_1.asm farcalls them too - AUDIO_1 (bank $02) has almost no ROM headroom
; and can't afford its own copy of this logic (unlike AUDIO_3/4, which keep
; their own inline copies since those banks have plenty of room). This is the
; one place this port deviates from "every bank keeps its own copy": one real
; copy, shared by farcall, costs far less ROM than duplicating ~280 bytes into
; a bank with ~16 bytes free.
...
.notNoiseChannel
	...
	ret
.playChannel
	call Audio2_InitSFXVariables ; same-bank call - no Bankswitch, bc/c survives
	ld a, c
	and a
...
; Input: de = software channel ID * 1 (doubled internally as needed).
; Clobbers af/bc/de/hl. Callers that farcall this (engine_1.asm) must
; save/restore anything they still need across the call themselves.
