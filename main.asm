...
; The curated MSRC_SET corpus, generated by tools/gen_movesets.py. Unlike the
; four files above, this does NOT share RogueBuildParty's bank - at ~15 KB it
; cannot (bank $39/"Trainer Parties" has only ~10 KB free), so it declares its
; own three pinned sections (see data/trainers/movesets.asm's own SECTION
; lines) and is read through a farcall, not a plain [hli] walk.
;
; Included AFTER rogue_build_party.asm, not before: this file's own SECTION
; directives become the "currently open" section for whatever follows, and
; rogue_build_party.asm has no SECTION of its own - it inherits "Trainer
; Parties" from above. Including movesets.asm first silently swept
; RogueBuildParty into "Movesets Index Tail" instead, which is exactly the
