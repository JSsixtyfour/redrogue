...
; MSRC_SET falls back to MSRC_RANDOM. Today that is unconditional, because the
; curated corpus is not generated yet (see data/moves/move_ranks.asm for what
; tools/gen_movesets.py owes this path). It stays a fallback afterwards: the
; corpus tier distribution is skewed enough - HARD 1,204 of 1,946, EASY 114,
; BAD 26 - that a narrow tier mask on an uncommon species legitimately yields
; zero candidates, and the alternative to falling back is reading past the
; species index.
	ld a, MSRC_RANDOM
	ld [wPartyGenSource], a
	; fallthrough
...
.pickLoop
	...
	pop bc
	and a
	ret z                          ; pool exhausted; the rest stay NO_MOVE
	push bc
...
; CLOBBERS af, hl. PRESERVES bc/de.
;
; ⚠ INERT UNTIL THE FLAGS TABLE LANDS. PartyGenMoveFlags returns 0 for every
; move, so nothing is ever forbidden. The code path is wired up and exercised on
; purpose, so dropping in the generated table is a one-routine change with no
; call-site edits.
...
;
; ⚠ STUB, FINAL INTERFACE. tools/reorder_move_ranks.py must emit a
; MoveFlagsByID table (2 bytes per move id) alongside MoveRankByID - the
; CORRECTED source file already carries the masks, as
; `db rank_code, LOW(flag_mask), HIGH(flag_mask)`. Replace this body with the
; indexed read; every caller already treats it as (in: e, out: de).
;
; Returning 0 makes `forbid_flags` inert and `require_flags` unsatisfiable,
; which is the correct degradation: the require semantics are "guarantee a move
; carrying that flag WHEN A LEGAL CANDIDATE EXISTS", and with no flag data no
; candidate carries one. require_flags is therefore not implemented here at all
; rather than half-implemented; see PARTY_SPEC_SPEC.md for where it hooks in.
PartyGenMoveFlags:
	ld d, 0
	ld e, 0
	ret
