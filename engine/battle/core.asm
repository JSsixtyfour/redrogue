...
LoadBattleMonFromParty:
	...
	ld [wCurSpecies], a
	; Phase 2R: wBattleMon already holds the switched-in mon (copied just above),
	; so its form bits are live here. Publish before the header load or a formed
	; player mon fights with its BASE species' stats, types and sprite.
	ld hl, wBattleMon
...
LoadEnemyMonFromParty:
	...
	ld [wCurSpecies], a
	ld hl, wEnemyMon ; Phase 2R: form bits arrived with the struct copy above
	call PublishFormContext
	call GetMonHeader
...
; enemy mon is not minimized
	...
	ld [wCurPartySpecies], a
	ld [wCurSpecies], a
	ld hl, wEnemyMon ; Phase 2R: keep the form on a mid-battle pic reload
	call PublishFormContext
	call GetMonHeader
	ld de, vFrontPic
	call LoadMonFrontSprite
...
.notLinkBattle
	...
	ld a, [wEnemyMonSpecies]
	ld [wCurSpecies], a
	ld hl, wEnemyMon ; Phase 2R: form-correct base stats for the DV/stat calc
	call PublishFormContext
	call GetMonHeader
	ld hl, wEnemyMonDVs
