...
; Pewter City events
    ...
    const EVENT_BEAT_PEWTER_GYM_TRAINER_2
    const EVENT_BEAT_PEWTER_GYM_TRAINER_3
	const_skip 3
...
; Cerulean City events
    ...
    const EVENT_BEAT_CERULEAN_GYM_TRAINER_3
	const_skip 2
...
; Lavender Town events
	...
	const EVENT_BEAT_POKEMONTOWER_7_TRAINER_3
	const EVENT_BEAT_POKEMONTOWER_7_TRAINER_4
	const_skip
	const EVENT_RESCUED_MR_FUJI_2
; Pokemon Tower 2F rogue trainers (fit in $119-$11D)
	const_skip 1
	const EVENT_BEAT_POKEMON_TOWER_2F_TRAINER_0
	const EVENT_BEAT_POKEMON_TOWER_2F_TRAINER_1
	const EVENT_BEAT_POKEMON_TOWER_2F_TRAINER_2
	const EVENT_BEAT_POKEMON_TOWER_2F_TRAINER_3
	const EVENT_BEAT_POKEMON_TOWER_2F_TRAINER_4
	const_skip 10
...
; Cinnabar Island events
	...
	const EVENT_BEAT_MANSION_1_TRAINER_4
	const_skip 10
...
; Route 1 rogue trainers (using Route 1 section gap)
	const_next $3C9
...
; Underground Path Route 5 rogue trainers
	const_next $3D1
	const EVENT_BEAT_UNDERGROUND_PATH_ROUTE5_TRAINER_0
	const EVENT_BEAT_UNDERGROUND_PATH_ROUTE5_TRAINER_1
	const EVENT_BEAT_UNDERGROUND_PATH_ROUTE5_TRAINER_2
	const EVENT_BEAT_UNDERGROUND_PATH_ROUTE5_TRAINER_3
	const EVENT_BEAT_UNDERGROUND_PATH_ROUTE5_TRAINER_4
...
; ============================================================================
; Red Rogue run-scoped EVENT, aliased onto one of Route 17's dead trainer bits.
;
; Only genuine milestones belong here. The earned stat boosts and the permanent
; witch prizes were briefly aliased into this block too; they are ordinary run
; state, not events, so they moved to wEarnedStatBoosts / wWitchPrizesEarned in
; ram/wram.asm.
;
...
;
; ALIASED rather than renamed/deleted because scripts/Route17.asm is still
; assembled (maps.asm) and uses the consts above in five  macros under
; , which hard-asserts each one's exact bit position. Deleting
; them breaks the build; renumbering shifts every downstream event.
;  costs nothing: NUM_EVENTS and wEventFlags are both unchanged.
;
; !!! IF ROUTE 17 IS EVER RE-ENABLED AS A STAGE, THIS BIT COLLIDES. !!!
; Beating its trainer 7 would silently mark Victory Road cleared, with no
; compile-time warning. Give Route 17 fresh event constants first. The same
; warning is on the commented-out  line in RogueStageMapTable.
;
; Cleared on blackout by RogueOnBlackout (custom_functions/credit_popup.asm).
; ============================================================================
; Moved out of wElite4Flags (was BIT_VICTORY_ROAD_CLEARED) so a blackout revokes
; the Victory Road clear. BIT_STARTED_ELITE_4 stays in wElite4Flags.
DEF EVENT_VICTORY_ROAD_CLEARED    EQU EVENT_BEAT_ROUTE_17_TRAINER_7
; Route 17 trainers 0-6, 8 and 9 remain spare for future run-scoped events.
...
; S.S. Anne events
	...
	const EVENT_BEAT_SS_ANNE_5_TRAINER_3
	const EVENT_BEAT_SS_ANNE_5_TRAINER_4
	const_skip 26
...
; End of events
; Rogue stage trainer events for maps with no existing trainers

; Diglett's Cave rogue trainers
	const_next $9E1
	const EVENT_BEAT_DIGLETTS_CAVE_TRAINER_0
	const EVENT_BEAT_DIGLETTS_CAVE_TRAINER_1
	const EVENT_BEAT_DIGLETTS_CAVE_TRAINER_2
	const EVENT_BEAT_DIGLETTS_CAVE_TRAINER_3
	const EVENT_BEAT_DIGLETTS_CAVE_TRAINER_4

; Seafoam Islands 1F rogue trainers
	const_next $9E9
	const EVENT_BEAT_SEAFOAM_ISLANDS_1F_TRAINER_0
	const EVENT_BEAT_SEAFOAM_ISLANDS_1F_TRAINER_1
	const EVENT_BEAT_SEAFOAM_ISLANDS_1F_TRAINER_2
	const EVENT_BEAT_SEAFOAM_ISLANDS_1F_TRAINER_3
	const EVENT_BEAT_SEAFOAM_ISLANDS_1F_TRAINER_4

; Silph Co 1F rogue trainers
	const_next $9F1
	...
	const EVENT_BEAT_SILPH_CO_1F_TRAINER_4

; SS Anne B1F rogue trainers
	const_next $9F9
...
; Rogue stage entrance auto-walk / no turning back flags
	const EVENT_AUTOWALKED_INTO_ROUTE_3
	const EVENT_AUTOWALKED_INTO_ROUTE_5
	const EVENT_AUTOWALKED_INTO_ROUTE_6
	const EVENT_AUTOWALKED_INTO_ROUTE_9
	const EVENT_AUTOWALKED_INTO_ROUTE_12
	const EVENT_AUTOWALKED_INTO_ROUTE_13
	const EVENT_AUTOWALKED_INTO_ROUTE_15
	const EVENT_AUTOWALKED_INTO_ROUTE_17
	const EVENT_AUTOWALKED_INTO_ROUTE_24
	const EVENT_AUTOWALKED_INTO_ROUTE_25
	const EVENT_AUTOWALKED_INTO_VIRIDIAN_FOREST
	const EVENT_AUTOWALKED_INTO_DIGLETTS_CAVE
	const EVENT_AUTOWALKED_INTO_MT_MOON_1F
	const EVENT_AUTOWALKED_INTO_ROCK_TUNNEL_1F
	const EVENT_AUTOWALKED_INTO_ROCKET_HIDEOUT_B1F
	const EVENT_AUTOWALKED_INTO_POKEMON_TOWER_2F
	const EVENT_AUTOWALKED_INTO_POKEMON_TOWER_7F
	const EVENT_AUTOWALKED_INTO_SS_ANNE_B1F
	const EVENT_AUTOWALKED_INTO_SS_ANNE_BOW
	const EVENT_AUTOWALKED_INTO_POWER_PLANT
	const EVENT_AUTOWALKED_INTO_POKEMON_MANSION_1F
