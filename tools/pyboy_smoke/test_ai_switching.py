...
class AISelectSendOutTest(unittest.TestCase):
    ...
    def call_selector_with_sentinels(self) -> tuple[int, int, list[int]]:
        assert self.harness is not None
        borrowed_labels = ("wEnemyMonType1", "wEnemyMonType2", "wPlayerMoveType")
        sentinels = [0xA1, 0xB2, 0xC3]
        for label, value in zip(borrowed_labels, sentinels):
            self.harness.write8(label, value)

        self.harness.call_routine("AISelectSendOut")

        restored = [self.harness.read8(label) for label in borrowed_labels]
        return (
    ...
    def test_excludes_active_mon_and_restores_borrowed_state(self) -> None:
        ...
        selected, multiplier, restored = self.call_selector_with_sentinels()

        self.assertEqual(selected, 1)
        self.assertEqual(multiplier, 20)
        self.assertEqual(restored, [0xA1, 0xB2, 0xC3])
