	object_const_def
	const_export ROUTE25_YOUNGSTER
	const_export ROUTE25_LASS
	const_export ROUTE25_JR_TRAINER_M
	const_export ROUTE25_HIKER
	const_export ROUTE25_COOLTRAINER_M
    const_export ROUTE25_POKE_BALL
    const_export ROUTE25_ROGUE_REWARD_POKEBALL_1
    const_export ROUTE25_ROGUE_REWARD_POKEBALL_2
    const_export ROUTE25_ROGUE_REWARD_POKEBALL_3
	const_export ROUTE25_TM_SEISMIC_TOSS

Route25_Object:
	db $2c ; border block

	def_warp_events
	warp_event  0,  9, LAST_MAP, 1
	warp_event  0,  8, LAST_MAP, 1
	warp_event 45,  3, INDIGO_PLATEAU_LOBBY, 1
	warp_event 43,  1, INDIGO_PLATEAU_LOBBY, 1

	def_bg_events
	bg_event 43,  3, TEXT_ROUTE25_BILL_SIGN

	def_object_events
	object_event 14,  6, SPRITE_YOUNGSTER, STAY, DOWN, TEXT_ROUTE25_YOUNGSTER, OPP_YOUNGSTER, 1
	object_event 16,  2, SPRITE_COOLTRAINER_F, STAY, DOWN, TEXT_ROUTE25_LASS, OPP_LASS, 1
	object_event 20,  7, SPRITE_COOLTRAINER_M, STAY, UP, TEXT_ROUTE25_JR_TRAINER_M, OPP_JR_TRAINER_M, 1
	object_event 27,  4, SPRITE_HIKER, STAY, DOWN, TEXT_ROUTE25_HIKER, OPP_HIKER, 1
	object_event 37,  4, SPRITE_COOLTRAINER_M, STAY, LEFT, TEXT_ROUTE25_COOLTRAINER_M, OPP_COOLTRAINER_M, 1
	object_event  9,  6, SPRITE_POKE_BALL, STAY, NONE, TEXT_ROUTE25_RANDOM, 0
	object_event 43,  6, SPRITE_POKE_BALL, STAY, NONE, TEXT_ROUTE25_ROGUE_REWARD_POKEBALL_1
	object_event 45,  6, SPRITE_POKE_BALL, STAY, NONE, TEXT_ROUTE25_ROGUE_REWARD_POKEBALL_2
	object_event 47,  6, SPRITE_POKE_BALL, STAY, NONE, TEXT_ROUTE25_ROGUE_REWARD_POKEBALL_3
	object_event 22,  2, SPRITE_POKE_BALL, STAY, NONE, TEXT_ROUTE25_TM_SEISMIC_TOSS, TM_SEISMIC_TOSS

	def_warps_to ROUTE_25
